...
export function ProfileSettings({snapshot,onSettings,onRefresh,onHistoryCleared,onError,notify,onConnections,onPlugins}: SettingsSectionProps & {onConnections:()=>void;onPlugins:()=>void}) {
  ...
  return <div className="local-profile-page">
    ...
    <section className="profile-section"><h4>Your workspace</h4><button className="profile-link" onClick={onConnections}><span>Models & connections<small>{snapshot.providers.filter(p=>p.available).length} detected connections</small></span><ArrowUpRight size={16}/></button><button className="profile-link" onClick={onPlugins}><span>Plugins & skills<small>Manage capabilities for your agents</small></span><ArrowUpRight size={16}/></button></section>
    ...
    <section className="profile-section"><h4>History</h4><p className="profile-footnote">Clear conversations and projects from the sidebar. Files, connections and message history stay on this Mac; you can restore them here.</p><div className="profile-history-actions"><button ref={clearButton} className="secondary-button" disabled={busy} onClick={()=>void perform(async()=>{const result=await api<HistoryArchiveReceipt>('history:archive');await onHistoryCleared?.();await onRefresh();await refresh();requestAnimationFrame(()=>clearButton.current?.focus());notify(`${result.tasks} conversations archived. You can restore them here.`)})}><Archive size={15}/>Clear history</button><button className="secondary-button" disabled={busy || !summary?.canRestoreHistory} onClick={()=>void perform(async()=>{const result=await api<HistoryArchiveReceipt>('history:restore');await onRefresh();await refresh();requestAnimationFrame(()=>clearButton.current?.focus());notify(`${result.tasks} conversations restored`)})}><Undo2 size={15}/>Restore last clear</button></div></section>
  </div>
}
