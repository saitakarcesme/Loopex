    ...
    return this.operation(async () => {
      ...
      try {
        ...
        for (const source of sources) {
          ...
          const header = source.kind === "instructions"
            ? `Project instructions: ${source.name}\nSource: ${source.path}\n`
            : `Akorith skill instructions: ${source.name}\nSource: ${source.path}\nThese instructions are already supplied below as context text. Apply them directly when relevant; do not call a native skill loader to load this source by name. Akorith has not registered this source in the provider's native skill catalog. Use tools mentioned here only when exposed by the actual tool catalog; report unavailable tools honestly.\n`;
